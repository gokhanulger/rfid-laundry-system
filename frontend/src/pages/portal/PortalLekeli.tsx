...
import { Droplet, Loader2, Search, ChevronLeft } from 'lucide-react';
import { itemsApi } from '../../lib/api';
import type { Item } from '../../types';

export function PortalLekeli() {
  const [search, setSearch] = useState('');

  const { data: stained, isLoading } = useQuery({
    queryKey: ['portal', 'stained-items'],
    queryFn: () => itemsApi.getStained(), // backend otel kullanicisini kendi tenant'ina kisitlar
  });
    ...
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Camasirhaneye lekeli olarak ulasan ve isaretlenen urunleriniz.
        </p>
      </div>
      ...
      {filtered.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {Object.entries(byType).map(([name, count]) => (
            <div key={name} className="bg-white rounded-xl border border-amber-100 p-4">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        {isLoading ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500 border-b border-gray-100">
                  ...
                  <th className="px-4 py-3 font-medium">Lekeli Tarihi</th>
                ...
                {filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-600">
                      {item.stainedAt ? format(new Date(item.stainedAt), 'dd MMM yyyy HH:mm', { locale: tr }) : '-'}
                    </td>
                    <td className="px-4 py-3 text-gray-600">{item.stainedReason || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="p-8 text-center">
            <Droplet className="w-12 h-12 text-gray-300 mx-auto" />
            <p className="mt-2 text-gray-500">Lekeli urun bulunamadi</p>
          </div>
        )}
      </div>
    </div>
