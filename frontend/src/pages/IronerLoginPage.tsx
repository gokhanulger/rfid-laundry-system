...
// Storage key for ironer authentication
const IRONER_AUTH_KEY = 'laundry_ironer_auth';
const IRONER_PIN = '1234'; // Default PIN

export function IronerLoginPage() {
  ...
  useEffect(() => {
    const savedAuth = localStorage.getItem(IRONER_AUTH_KEY);
    if (savedAuth === 'true') {
  ...
  const handlePinInput = (digit: string) => {
    if (pin.length < 4) {
      ...
      if (newPin.length === 4) {
        setTimeout(() => {
          if (newPin === IRONER_PIN) {
            localStorage.setItem(IRONER_AUTH_KEY, 'true');
            setIsAuthenticated(true);
          } else {
  ...
  // If authenticated, show the ironer interface
  if (isAuthenticated) {
    return <IronerInterfacePage />;
  }
