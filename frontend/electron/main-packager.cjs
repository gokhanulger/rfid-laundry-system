...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    title: 'RFID Paketleme İstasyonu'
  });

  // Load the app - directly to packager login page
  if (isDev) {
    // Development: Load from Vite dev server
    mainWindow.loadURL('http://localhost:3002/#/packager');
    mainWindow.webContents.openDevTools();
  } else {
    // Production: Load from built files using app path
    const appPath = app.getAppPath();
    mainWindow.loadFile(path.join(appPath, 'dist/index.html'), { hash: '/packager' });
